...
exports.combine = function (config) {
    ...
    /**
     * Remove matching quotes from a string if they exist.
     */
    function unquote (string) {
        var quotes = string.match(/^'(.*)'$|^"(.*)"$/);
        if (quotes) {
            // Path could be either first or second capture
    ...
    function absolutizeResources(absolutePath, data) {
        ...
        absolutelyRelativePath = path.dirname(absolutePath.replace(basePath, ''));
        ...
        if (urlRegex.test(data)) {
            ...
            */
            data = data.replace(urlRegex, function (match, filepath) {
                var replacement = unquote(filepath);

                // don't whack data URIs
                if (replacement.indexOf('data:') === -1) {
                    replacement = path.resolve(absolutelyRelativePath, replacement);
                }
        ...
        if (importRegex.test(data)) {
            data = data.replace(importRegex, function (match, filepath, mediaQuery) {
                var replacement = unquote(filepath);

                // urls have already been replaced in the first pass
                if (!urlRegex.test(replacement)) {
                    replacement = '"' + path.resolve(absolutelyRelativePath, replacement) + '"';
                }

                // Preserve imports with media queries i.e. @import print.css print;
                if (mediaQuery) {
                    return '@import ' + replacement + mediaQuery + ';';
                } else {
...
function getDynamicKey(rootPath) {
    ...
    var keyRegex = /:\w+/;
    return keyRegex.test(rootPath) && rootPath.match(keyRegex)[0];
}
