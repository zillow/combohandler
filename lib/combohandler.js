...
// -- Module Exports -----------------------------------------------------------
exports = module.exports = combohandler;

// exported to allow instanceof checks
var BadRequest = exports.BadRequest = require('./error/bad-request');
...
exports.respond = respondMiddleware;

// -- Exported Methods ---------------------------------------------------------
function combohandler(config) {
    maybeCombo.callbacks = combine(config);

    function maybeCombo(req, res, next) {
        if (req.method !== 'GET') {
            // don't error, pass to next middleware in stack
            return next();
        }

        function maybeSend(err) {
            if (err) {
                next(err);
            } else {
                if (req.route) {
        ...
        if (maybeCombo.callbacks.length === 1) {
            maybeCombo.callbacks[0](req, res, maybeSend);
        } else {
            // loop through callbacks
            var i = 0;
            (function nextCallback(err) {
                var fn = maybeCombo.callbacks[i++];
                try {
                    if (err) {
                        maybeSend(err);
                    } else if (fn) {
...
function respondMiddleware(req, res) {
    res.send(res.body);
}
