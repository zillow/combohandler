...
var ComboCluster = module.exports = function ComboCluster(config) {
    ...
    this.options = libutil.merge(ComboCluster.defaults, config);
};
...
ComboCluster.prototype = {
    ...
    _listen: function () {
        if (cluster.isMaster) {
            ...
            // listen
            var server = require(this.options.server);
            server(this.options).listen(this.options.port);
        }
    },
